...
//<--- SCENE CREATION --->
	...
	m_pPlayer = BuildCuboidObject(
		...
		Vector3(5.f, 2.0f, 0.0f),	// Position
	...
	GameObject* sphere2 = BuildSphereObject(
		"",					// Optional: Name
		Vector3(0, 25, 20),				// Position
		5.0f,			// Half-Dimensions
		true,				// Physics Enabled?
		0.0f,				// Physical Mass (must have physics enabled)
		true,				// Physically Collidable (has collision shape)
		true,				// Dragable by user?
		Vector4(1.0, 0.5, 1.0, 0.5));// Render color

	this->AddGameObject(sphere2);

	GameObject* sphere3 = BuildSphereObject(
		"",					// Optional: Name
		Vector3(-20, 25, 20),				// Position
		5.0f,			// Half-Dimensions
		true,				// Physics Enabled?
	...
	//PhysicsEngine::Instance()->AddConstraint(new DistanceConstraint(sphere->Physics(), FindGameObject("Player1")->Physics(), Vector3(0, 0, 0), Vector3(0, 0, 0)));
	PhysicsEngine::Instance()->AddConstraint(new DistanceConstraint(sphere->Physics(), sphere2->Physics(), sphere->Physics()->GetPosition(), sphere2->Physics()->GetPosition()));
	PhysicsEngine::Instance()->AddConstraint(new DistanceConstraint(sphere3->Physics(), sphere2->Physics(), sphere3->Physics()->GetPosition(), sphere2->Physics()->GetPosition()));

	GenerateSoftBody();
	GenerateSoftBody(5,10,true,Vector3(10,0,0));
...
//Builds a cloth like soft body using string constraints and spheres
void TestScene::GenerateSoftBody(int width, int height,bool fixed, Vector3 pos)
{
	vector<vector<GameObject*>> spheres(width, vector<GameObject*>(height,NULL));
	...
	for (int i = 0; i < width; i++) {
		for (int j = 0; j < height; j++) {
			...
			if ((i == 0 || i == width - 1) && j == height - 1 && fixed) {
				mass = 0.0f;
			}

			GameObject* sphere = BuildSphereObject(
				"",					// Optional: Name
				Vector3(i * 2, (j * 2) + height, 0) + pos,				// Position
				0.5f,			// Half-Dimensions
				true,				// Physics Enabled?
				mass,				// Physical Mass (must have physics enabled)
				true,				// Physically Collidable (has collision shape)
				true,				// Dragable by user?
				Vector4(1, 1, 1, 1));// Render color
	...
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < 10; j++) {
			...
			if (i + 1 < width) {
				PhysicsEngine::Instance()->AddConstraint(new SpringConstraint(spheres[i][j]->Physics(), spheres[i+1][j]->Physics(), spheres[i][j]->Physics()->GetPosition(), spheres[i + 1][j]->Physics()->GetPosition()));
			}

			if (j + 1 < height) {
				PhysicsEngine::Instance()->AddConstraint(new SpringConstraint(spheres[i][j]->Physics(), spheres[i][j + 1]->Physics(), spheres[i][j]->Physics()->GetPosition(), spheres[i][j + 1]->Physics()->GetPosition()));
			}
