...
__global__
void CollideParticles(float baumgarte_factor, uint num_particles, Particle* particles, Particle* out_particles, uint* grid_cell_start, uint* grid_cell_end, float3 ballPos, float3 ballVel)
{
	...
	float3 ab = ballPos - p._pos;
	float lengthSq = dot(ab, ab);

	const float diameterSq = (BALL_RADIUS + PARTICLE_RADIUS) * (BALL_RADIUS + PARTICLE_RADIUS);
	if (lengthSq < diameterSq)
	{
		//We have a collision!
		float len = sqrtf(lengthSq);
		...
		//Direct normal collision (no friction/shear)
		float abnVel = dot(ballVel - p._vel, abn);
		float jn = -(abnVel * (1.f + COLLISION_ELASTICITY));

		//Extra energy to overcome overlap error
		float overlap = PARTICLE_RADIUS + BALL_RADIUS - len;
		float b = overlap * baumgarte_factor;
...
void CudaCollidingParticles::UpdateParticles(float dt, float3 ballPos, float3 ballVel)
{
	...
	for (int i = 0; i < 10; ++i)
	{
		CollideParticles<<< grid, block >>>(baumgarte_factor, num_particles, particles_ping, particles_pong, grid_cell_start, grid_cell_end, ballPos, ballVel);
