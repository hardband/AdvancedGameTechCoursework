...
MazeRenderer::MazeRenderer(FlatMazeDescriptor flatDes)
{
	this->SetRender(new RenderNode());

	//flat_maze = flatDes.maze;
	flat_maze_size = flatDes.size;
...
void MazeRenderer::DrawSearchHistory(string path, float line_width)
{
	...
	for (int i = 0; i < path.size(); i++) {
		...
		pos = Vector3(stof(x),stof(y),stof(z));
	...
	for (int i = 0; i < nodes.size() - 1; i++) {
		...
		element.first = nodes[i];
		element.second = nodes[i + 1];
		history.push_back(element);
	}

	DrawSearchHistory(history,line_width);
...
FlatMazeDescriptor MazeRenderer::GetFlatMazeDescriptor()
{
	FlatMazeDescriptor des;
	//des.maze = flat_maze;
	des.maze.resize(flat_maze_size * flat_maze_size);

	for (int i = 0; i < flat_maze_size * flat_maze_size; i++) {
		des.maze[i] = flat_maze[i];
	}
	des.size = flat_maze_size;
	return des;
}

FlatMazeDescriptor MazeRenderer::MakeFlatMaze(MazeGenerator * maze)
{
	...
	bool* flat_maze = new bool[flat_maze_size * flat_maze_size];
	memset(flat_maze, 0, flat_maze_size * flat_maze_size * sizeof(bool));
...
Vector3 MazeRenderer::TranslateToWorld(Vector3 pos)
{
	float grid_scalar = 1.0f / (float)maze->GetSize();
	
	Matrix4 transform = this->Render()->GetWorldTransform();

	return transform * Vector3((pos.x + 0.5f) * grid_scalar,0.1f, (pos.y + 0.5f) * grid_scalar);
}
