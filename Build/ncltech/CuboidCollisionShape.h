...
public:
	...
	void SetHalfWidth(float half_width) { halfDims.x = fabs(half_width); calculateRadius(); }
	void SetHalfHeight(float half_height) { halfDims.y = fabs(half_height);calculateRadius();}
	void SetHalfDepth(float half_depth) { halfDims.z = fabs(half_depth);calculateRadius(); }
