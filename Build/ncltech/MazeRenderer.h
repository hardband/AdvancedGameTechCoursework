...
public:
	...
	MazeRenderer(FlatMazeDescriptor flatDes);
	//MazeRenderer(WallDescriptorVec descriptors);
	...
	void DrawSearchHistory(string path, float line_width);


	FlatMazeDescriptor GetFlatMazeDescriptor();

	static FlatMazeDescriptor MakeFlatMaze(MazeGenerator* maze);

	Vector3 TranslateToWorld(Vector3 pos);


	//WallDescriptorVec GetDescriptors();
